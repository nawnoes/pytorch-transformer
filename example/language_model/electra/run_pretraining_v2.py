    ...
    for epoch in range(start_epoch, epochs):  # tqdm(range(epochs), desc='Epochs', position=0):
      ...
      for step, batch in pb:
        ...
        if global_steps % log_steps == 0:
          pb.set_postfix_str(
            f'''{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Train Loss: {format(step_loss / local_steps, ".4f")} | Steps: {global_steps} | disc_acc: {format(output.disc_acc, ".4f")} | disc_loss: {format(output.disc_loss, ".4f")} | gen_acc: {format(output.gen_acc, ".4f")} | mlm_loss: {format(output.mlm_loss, ".4f")} ''')
          step_loss = 0.0
          local_steps = 0
...
def main():
  ...
  config_path = f'{base_path}/config/electra/small/electra-train-v2.json'
  fp16 = True # Whether to use 16-bit float precision instead of 32-bit
  fp16_opt_level = '02' # For fp16: Apex AMP optimization level selected in ['O0', 'O1', 'O2', and 'O3']
